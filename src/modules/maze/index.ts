...
export default class extends Module {
	...
	private async mentionHook(msg: Message) {
		if (msg.includes(['迷宮', '迷路'])) {
			let size: string | null = null;
			if (msg.includes(['非常簡單', '接待'])) size = 'veryEasy';
			if (msg.includes(['簡單', '簡単', 'かんたん', '易しい', 'やさしい', '小さい', 'ちいさい'])) size = 'easy';
			if (msg.includes(['困難', '難しい', 'むずかしい', '複雑な', '大きい', 'おおきい'])) size = 'hard';
			if (msg.includes(['非常困難', '死', '鬼', '地獄'])) size = 'veryHard';
